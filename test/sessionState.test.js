...
test('client marks only running VNC browsers as interactive', () => {
  assert.equal(isInteractiveBrowser({ status: 'running', browserMode: 'vnc', vncActive: true }), true);
  assert.equal(isInteractiveBrowser({ status: 'running', browserMode: 'vnc', vncActive: false }), false);
  assert.equal(isInteractiveBrowser({ status: 'running', browserMode: 'headless', vncActive: false }), false);
  assert.equal(isInteractiveBrowser({ status: 'stopped', browserMode: 'vnc', vncActive: true }), false);
