...
async function execute(req, res) {
  ...
  const result = await bc.evaluateOnMain(pid, wrapped);
...
async function uploadFile(req, res) {
  ...
  const pid = String(req.params.id);
  await ensureConnected(pid);
  console.log(`[Upload] profile=${pid} file=${fileName} size=${Buffer.from(fileData, 'base64').length}`);

  // Write decoded file to host temp dir
  const tmpName = `hive-upload-${crypto.randomBytes(6).toString('hex')}-${fileName}`;
  ...
  try {
    ...
    console.log(`[Upload] copied to container: ${containerPath}, host size=${buf.length}`);

    // Verify file size in container
    const sizeOut = await cm.execInContainer(pid, ['stat', '-c', '%s', containerPath]);
    console.log(`[Upload] container file size: ${sizeOut.trim()}`);
    ...
    console.log('[Upload] xdotool done');
    res.json({ ok: true });
  } catch (err) {
