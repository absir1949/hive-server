 ...
 *
 * 每个操作端点的内部流程：
 *   ensureReady(profileId) → 加载 profile → 生成指纹扩展 → 启容器 → 连 CDP → 拿 page → 执行操作
 * 调用者不需要关心容器生命周期。
 *
...
/**
 * Ensure container running + CDP connected + return page.
 * Throws ApiError with proper status codes.
 */
async function ensureReady(profileId) {
  ...
  // Connect and get page, with retry on failure
  try {
    if (!bc.get(pid)) {
      await bc.connect(pid, info.cdpPort);
    }
    return await bc.getPage(pid);
  } catch (err) {
...
async function navigate(req, res) {
  ...
  const page = await ensureReady(req.params.id);
...
async function getVncUrl(req, res) {
  try {
    ...
    res.json({ ok: true, url: `http://${host}:${info.vncPort}/vnc.html` });
  } catch (err) {
    const apiErr = classifyError(err);
    res.status(apiErr.status).json({ ok: false, error: apiErr.message });
  }
