...
async function navigate(req, res) {
  ...
  // Use a background tab so we don't interrupt the user's active page.
  // Cookies are shared across tabs, so this still keeps the session alive.
  const { pageId, page } = await bc.newPage(pid, url);
  try {
    const finalUrl = page.url();
    res.json({ ok: true, url: finalUrl });
  } finally {
    await bc.closePage(pid, pageId).catch(() => {});
  }
}
