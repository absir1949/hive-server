...
const { maxRunningBrowsers, idleStopMs, collectionPageIdleMs, pickEvictionVictim } = require('./sessionCapacity');
...
function mount(app, deps) {
  ...
  app.post('/browsers/:id/cookies', browserOp(setCookies));
  ...
  app.get('/browsers/:id/pages', browserOp(listPages));
...
function errorPayload(apiErr) {
  ...
  if (Array.isArray(apiErr.pages)) body.pages = apiErr.pages;
  return body;
}
...
function resetIdleStopTimer(profileId) {
  ...
  const timer = setTimeout(() => {
    withProfileLock(pid, async () => {
      ...
      // A leaked pageId must not protect this browser forever: verify the
      // tracked pages still exist in Chrome before honoring the protection.
      await bc.reconcileCollectionPages?.(pid);
      await bc.closeIdleCollectionPages?.(pid);
      if (bc.hasOpenPages?.(pid)) {
        resetIdleStopTimer(pid);
        return;
      }
...
async function evictToCapacity(startingId) {
  await sweepIdleCollectionPages();
...
async function sweepIdleCollectionPages() {
  if (!collectionPageIdleMs() || typeof bc.closeIdleCollectionPages !== 'function') return;
  for (const id of [...cm.containers.keys()]) {
    await bc.reconcileCollectionPages?.(id);
    await bc.closeIdleCollectionPages(id);
  }
}
...
async function ensureConnected(profileId, requestedMode) {
  ...
  if (!info) {
    try {
      ...
      // Touch immediately so the periodic capacity sweep cannot treat the
      // just-started browser (lastUsedAt still unset) as the LRU victim.
      touchLastUsed(pid);
    } catch (err) {
...
async function acquireVncSession(profileId, requestedLeaseId) {
  ...
  if (active?.browserMode && normalizeBrowserMode(active.browserMode) === BROWSER_MODES.HEADLESS) {
    ...
    if (status === 'running') {
      // A leaked pageId must not block this explicit switch forever.
      await bc.reconcileCollectionPages?.(pid);
      await bc.closeIdleCollectionPages?.(pid);
    }
    if (status === 'running' && bc.hasOpenPages?.(pid)) {
      const pageIds = bc.collectionPageIds?.(pid) || [];
      throw new ApiError(
        409,
        `Profile ${profileId} has an active collection (pages: ${pageIds.join(', ') || 'unknown'}); `
          + `wait for it to finish or close it via POST /browsers/${profileId}/pages/{pageId}/close before opening VNC`,
        { pages: pageIds },
      );
    }
    if (status === 'running') {
...
/**
 * Write side of GET /cookies: seed a Profile's persistent cookie jar
 * (auth-cookies.json) and, when the browser is running, inject the same
 * cookies live via CDP. This lets callers transplant a login minted
 * elsewhere (e.g. a switchShop renewal on a sibling profile) into the
 * shop's own Profile without a manual VNC login.
 *
 * mode=merge (default) replaces by (name, domain) inside the existing dump;
 * mode=replace swaps the whole jar. No delete semantics on purpose.
 */
async function setCookies(req, res) {
  ...
  if (!cookies || cookies.length === 0) {
    throw new ApiError(400, 'cookies must be a non-empty array');
  }
  for (const c of cookies) {
    if (!c || typeof c.name !== 'string' || typeof c.value !== 'string' || typeof c.domain !== 'string') {
      throw new ApiError(400, 'each cookie needs string name, value and domain');
    }
  }
  ...
  if (mode === 'replace') {
    ...
    const dump = authStore.readDump(pid);
    const existing = Array.isArray(dump?.cookies) ? dump.cookies.slice() : [];
    for (const incoming of cookies) {
      const idx = existing.findIndex((c) => c.name === incoming.name && c.domain === incoming.domain);
      if (idx >= 0) existing[idx] = { ...existing[idx], ...incoming };
      else existing.push(incoming);
    }
    authStore.save(pid, existing);
  }
  ...
  const total = (authStore.readDump(pid)?.cookies || []).length;
  res.json({ ok: true, profileId: pid, mode, merged: cookies.length, total, liveInjected });
}
...
/**
 * Tracked collection pages for a running browser. Never cold-starts a
 * stopped one; there is nothing to list until bc connects again.
 */
async function listPages(req, res) {
  ...
  await bc.reconcileCollectionPages?.(pid);
  const pages = typeof bc.listCollectionPages === 'function'
    ? await bc.listCollectionPages(pid)
    : [];
  res.json({ ok: true, pages });
}
...
/**
 * Arm the idle-stop timer for every running container. Called after
 * recovery: without this, recovered browsers were never idle-stopped.
 */
function armIdleStopTimers() {
  for (const pid of cm.containers.keys()) touchLastUsed(pid);
}
