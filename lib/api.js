...
function resolveSessionMode(profileId, requestedMode) {
  ...
  if (active?.browserMode) return normalizeBrowserMode(active.browserMode);
  return sessionModes.get(pid) || BROWSER_MODES.VNC;
}
...
async function ensureVncConnected(profileId) {
  const requestedMode = resolveSessionMode(profileId);
  if (!isVncMode(requestedMode)) {
    throw new ApiError(409, `Profile ${profileId} is currently running in headless mode`);
  }
  const info = await ensureConnected(profileId);
