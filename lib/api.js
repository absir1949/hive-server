...
const { normalizeBrowserMode, isVncMode } = require('./browserMode');
...
async function ensureConnected(profileId) {
  ...
  startOptions.browserMode = normalizeBrowserMode(profile.browserMode);
  ...
  try {
    ...
    list = list.map((b) => {
      ...
      return {
        ...
        name: profile?.name,
        type: profile?.type,
        browserMode: profile?.browserMode || b.browserMode,
      };
    });
    if (typeFilter) {
...
async function getVncUrl(req, res) {
  try {
    ...
    res.json({ ok: true, url: `http://${host}:${info.vncPort}/vnc.html?resize=scale&autoconnect=true` });
  } catch (err) {
    const apiErr = classifyError(err);
    res.status(apiErr.status).json({ ok: false, error: apiErr.message });
  }
...
async function createProfile(req, res) {
  try {
    ...
    try {
      normalizeBrowserMode(req.body.browserMode);
    } catch (err) {
...
async function updateProfile(req, res) {
  try {
    ...
    const previousMode = normalizeBrowserMode(existing.browserMode);
    let nextMode;
    try {
      nextMode = normalizeBrowserMode(req.body.browserMode === undefined
        ? previousMode
        : req.body.browserMode);
    } catch (err) {
      return res.status(400).json({ ok: false, error: err.message });
    }

    // A user-data-dir must not be opened by VNC and headless Chrome at the
    // same time. Stop the old container before persisting a mode change.
    if (previousMode !== nextMode) {
      await stopProfileContainer(req.params.id);
    }
...
function requireVncMode(profileId) {
  const profile = ps.get(profileId);
  if (!profile) {
    throw new ApiError(404, `Profile ${profileId} not found`);
  }
  if (!isVncMode(profile.browserMode)) {
    throw new ApiError(409, `Profile ${profileId} uses headless mode and has no VNC session`);
  }
}
