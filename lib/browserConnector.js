...
class BrowserConnector {
  ...
  async getMainPage(profileId) {
    ...
    if (mainTarget) {
      // Map CDP target to Puppeteer page (try URL first, then targetId)
      const pages = await browser.pages();
      let page = pages.find((p) => p.url() === mainTarget.url);
      if (!page) page = pages.find((p) => p.target()._targetId === mainTarget.id);
      ...
      if (!page) {
        ...
        if (!page) page = retryPages.find((p) => p.target()._targetId === mainTarget.id);
      }
  ...
  async closePage(profileId, pageId) {
    ...
    // Try Puppeteer first (page.close() only closes the tab, not the browser)
    try {
