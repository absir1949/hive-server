...
const { collectionPageIdleMs } = require('./sessionCapacity');
...
class BrowserConnector {
  ...
  collectionPageIds(profileId) {
    return [...(this.collectionPages.get(String(profileId))?.keys() || [])];
  }

  /**
   * Tracked collection pages with their current URL (best effort).
   */
  async listCollectionPages(profileId) {
    const pid = String(profileId);
    const pageIds = this.collectionPageIds(pid);
    ...
    if (conn) {
      try {
        ...
        urls = new Map(targets.map((t) => [String(t.id), t.url]));
      } catch { /* URL enrichment is best-effort */ }
    }
    return pageIds.map((pageId) => ({ pageId, url: urls.get(pageId) || null }));
  }

  /**
   * Drop tracked collection pages whose target no longer exists in Chrome.
   * A leaked pageId would otherwise protect its browser from idle stop and
   * eviction forever. Fails safe: when Chrome cannot be queried, tracked
   * pages stay protected.
   */
  async reconcileCollectionPages(profileId) {
    const pid = String(profileId);
    ...
    let targets;
    try {
      targets = await this._cdpGet(conn.cdpPort, '/json/list');
    } catch (err) {
      console.error(`[BrowserConnector] Cannot reconcile collection pages for ${pid}: ${err.message}`);
      return;
    }
    const alive = new Set(targets.map((t) => String(t.id)));
    for (const [pageId, page] of [...pages.entries()]) {
      if (alive.has(pageId)) continue;
      console.log(`[BrowserConnector] Pruning stale collection page ${pageId} of ${pid}`);
      page.ws.terminate?.();
  ...
  /**
   * Replace cookies in the running browser. Callers should reload the
   * authenticated origin afterwards so the new cookies take effect.
   */
  async setCookiesMain(profileId, cookies) {
    const { wsUrl } = await this._getMainPageWs(profileId);
    const ws = await this._connectWs(wsUrl);
    try {
      await this._cdpSend(ws, 'Network.enable');
      await this._cdpSend(ws, 'Network.setCookies', { cookies });
    } finally {
      ws.close();
    }
  }
  ...
  _touchCollectionPage(profileId, pageId) {
    const page = this._getCollectionPage(profileId, pageId);
    if (page) page.lastActivityAt = Date.now();
    return page;
  }

  /**
   * Close collection pages with no CDP activity for idleMs.
   * Live tasks that still execute/screenshot keep their lastActivityAt fresh.
   */
  async closeIdleCollectionPages(profileId, idleMs) {
    const limit = idleMs == null ? collectionPageIdleMs() : idleMs;
    if (!limit) return [];
    ...
    const closed = [];
    for (const [pageId, page] of [...pages.entries()]) {
      const last = page.lastActivityAt || page.createdAt || 0;
      if (now - last < limit) continue;
      const idleFor = now - last;
      console.log(
        `[BrowserConnector] Closing idle collection page ${pageId} of ${pid} after ${Math.round(idleFor / 1000)}s with no CDP activity`,
      );
      try {
        await this.closePage(pid, pageId);
        closed.push(pageId);
      } catch (err) {
        console.error(
          `[BrowserConnector] Failed to close idle collection page ${pageId} of ${pid}: ${err.message}`,
        );
      }
    }
    return closed;
  ...
  async evaluateOnPage(profileId, pageId, expression) {
    ...
    const collectionPage = this._touchCollectionPage(pid, pageId);
    if (collectionPage) {
