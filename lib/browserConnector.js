...
class BrowserConnector {
  ...
  async insertTextMain(profileId, text) {
    ...
    try {
      ...
      console.log('[Clipboard] insertText done, firing input/change events');

      // End IME composition and fire input/change events so page frameworks
      // (React/Vue) sync their internal state and the input exits edit mode.
      const evResult = await this._cdpSend(ws, 'Runtime.evaluate', {
        expression: `(() => {
          const el = document.activeElement;
          const info = { tag: el?.tagName, hasFocus: document.hasFocus() };
          if (el && (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.isContentEditable)) {
            el.dispatchEvent(new Event('input', { bubbles: true }));
            el.dispatchEvent(new Event('change', { bubbles: true }));
            info.eventsFired = true;
          }
          return JSON.stringify(info);
        })()`,
        returnByValue: true,
      });
      console.log('[Clipboard] post-insert state:', evResult.result?.value);

      // Dispatch Escape to dismiss any IME popup
      await this._cdpSend(ws, 'Input.dispatchKeyEvent', {
        type: 'keyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27,
      });
      await this._cdpSend(ws, 'Input.dispatchKeyEvent', {
        type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27,
      });
      console.log('[Clipboard] Escape dispatched');
      return true;
    } finally {
      ws.close();
    }
  ...
  /**
   * Set files on the first visible <input type="file"> via CDP.
   * `containerPaths` are paths inside the container filesystem.
   */
  async setFileInputFiles(profileId, containerPaths) {
    const { wsUrl, targetId } = await this._getFocusedPageWs(profileId);
    const ws = await this._connectWs(wsUrl);
    try {
      // Find the first visible file input and get its backendNodeId
      const { result } = await this._cdpSend(ws, 'Runtime.evaluate', {
        expression: `(() => {
      ...
      const desc = await this._cdpSend(ws, 'DOM.describeNode', {
