...
class BrowserConnector {
  ...
  /**
   * Get the main page — the user's active page with real content.
   * Uses CDP to discover all targets, since browser.pages() only tracks
   * targets that Puppeteer has seen (misses tabs opened by Chrome itself).
   */
  async getMainPage(profileId) {
    const browser = await this.ensureBrowser(profileId);

    // First try: find among Puppeteer-tracked pages
    const pages = await browser.pages();
    let page = pages.find(
      (p) => p.url() !== 'about:blank' && p.url() !== 'chrome://newtab/'
    );

    // Fallback: discover via CDP — Chrome may have tabs Puppeteer hasn't seen
    if (!page) {
    ...
    // Puppeteer sets default 800x600 viewport when attaching to existing targets.
    // Fix to match Xvfb display (1920x1080) if needed.
    try {
