  ...
  async recover() {
    ...
    for (const c of all) {
      if (c.status === 'running' && c.cdpPort) {
        // VNC leases live in server memory. After a restart there is no
        // trustworthy client lease to restore, so never leave an orphaned
        // interactive container running.
        if (c.browserMode === BROWSER_MODES.VNC) {
          await this.stop(c.profileId);
          removedVnc.push(c.profileId);
          continue;
        }
    ...
    if (removedVnc.length > 0) {
      console.log(`Removed ${removedVnc.length} stale VNC containers after restart: ${removedVnc.join(', ')}`);
    }
    return recovered;
  }
