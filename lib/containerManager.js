...
class ContainerManager {
  constructor({ docker } = {}) {
    ...
    // profileId → { containerId, cdpPort, vncPort, browserMode }
   ...
   * Remove only exited leftovers; never replace a running browser implicitly.
   */
  async start(profileId, options = {}) {
    const browserMode = normalizeBrowserMode(options.browserMode);

    // Already tracked and running
    const existing = this.containers.get(profileId);
    if (existing && (existing.browserMode || BROWSER_MODES.HEADLESS) === browserMode) {
      return existing;
    }
    if (existing) {
      throw new Error(
        `Profile ${profileId} is already running in ${existing.browserMode} mode; `
          + `explicitly stop it before starting ${browserMode} mode`,
      );
    }
    ...
    // Clean up an exited leftover container with the same name. A running but
    // untracked container may still hold session cookies and must never be
    // replaced implicitly.
    try {
      const old = this.docker.getContainer(containerName);
      const state = await old.inspect();
      if (state.State?.Running) {
        throw new Error(
          `Profile ${profileId} already has an untracked running container; `
            + 'recover it or explicitly stop it before starting a replacement',
        );
      }
      await old.remove();
    } catch (err) {
    ...
    env.push(`BROWSER_MODE=${browserMode}`);
    ...
    if (browserMode === BROWSER_MODES.VNC) {
      portBindings['6080/tcp'] = [{ HostPort: String(vncPort) }];
    }

    const memoryBytes = parseInt(process.env.CHROME_MEMORY_BYTES, 10) || 1536 * 1024 * 1024;
    const container = await this.docker.createContainer({
      Image: IMAGE,
      name: containerName,
      Env: env,
      Labels: { 'hive.profileId': profileId, 'hive.browserMode': browserMode },
      HostConfig: {
        ...
        NanoCpus: parseInt(process.env.CHROME_NANO_CPUS, 10) || 500000000,
      },
    });
    ...
    const info = {
      ...
      cdpPort,
      vncPort: browserMode === BROWSER_MODES.VNC ? vncPort : null,
      browserMode,
    };
    ...
    return allContainers
      ...
      .map((c) => {
        ...
        return {
          ...
          vncPort: ports.vncPort || null,
          browserMode: c.Labels?.['hive.browserMode'] || (ports.vncPort ? BROWSER_MODES.VNC : BROWSER_MODES.HEADLESS),
        };
      });
  }
  ...
  async recover() {
    ...
    for (const c of all) {
      if (c.status === 'running' && c.cdpPort) {
        this.containers.set(c.profileId, {
          containerId: c.containerId,
          cdpPort: c.cdpPort,
          vncPort: c.vncPort,
          browserMode: c.browserMode,
        });

        // VNC leases live in Server memory and cannot be recovered. Revoke
        // only the remote-control channel; Chromium is the durable auth
        // session and must keep running across a Server process restart.
        if (isVncMode(c.browserMode)) {
          await this.disableVncAccess(c.profileId);
          revokedVnc.push(c.profileId);
        }
    ...
    if (revokedVnc.length > 0) {
      console.log(`Revoked stale VNC access for ${revokedVnc.length} recovered containers: ${revokedVnc.join(', ')}`);
    }
    return recovered;
  }
  ...
  async enableVncAccess(profileId) {
    ...
    if (!info || !isVncMode(info.browserMode)) {
      throw new Error(`Profile ${profileId} is not running in VNC mode`);
    }
    await this.execInContainer(pid, ['/usr/local/bin/hive-vnc-control', 'start']);
  }

  async disableVncAccess(profileId) {
    const pid = String(profileId);
    const info = this.containers.get(pid);
    if (!info || !isVncMode(info.browserMode)) return false;
    try {
      await this.execInContainer(pid, ['/usr/local/bin/hive-vnc-control', 'stop']);
      return true;
    } catch (err) {
      ...
      stream.on('end', async () => {
        try {
          ...
          if (result.ExitCode !== 0) {
            reject(new Error(`Container command failed (${result.ExitCode}): ${output.trim()}`));
            return;
          }
          resolve(output);
        } catch (err) {
   ...
   */
  async shutdown() {
    for (const [profileId, info] of this.containers) {
      if (!isVncMode(info.browserMode)) continue;
      await this.disableVncAccess(profileId).catch((err) => {
        console.error(`[VNC] Failed to revoke access for ${profileId} during shutdown:`, err.message);
      });
    }
